...
const GEMINI_API_KEY = 'AIzaSyDz4WTP5LlWAYj-wtvRs0YgodO8RvztUx4';
...
"bp", "sugar", "medication", "chronic", "family", "surgeries", "complication".`;
        ...
        resultDiv.innerHTML = `<p style="color: #ef4444; font-weight: 600;">⚠️ Error: ${error.message}</p><p style="color: #64748b; margin-top: 8px;">Please check your API key and try again.</p>`;
...
window.viewPatientAsDoctor = async function(patientId, bypassPin = false) {
    ...
    try {
        ...
        if (patientData && (bypassPin || patientData.pin === pin)) {
            ...
            // Start 10-min session timer (only if not already inside org read-only mode)
            if (!window._readOnlyMode) {
        ...
        if (visit) {
            ...
            if (file.type.startsWith('image/') || file.type === 'application/pdf') {
                ...
                reader.onload = async () => {
                    try {
                        const base64Data = reader.result.split(',')[1];
                        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=AIzaSyDz4WTP5LlWAYj-wtvRs0YgodO8RvztUx4`, {
